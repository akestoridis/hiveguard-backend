  ...
  constructor(config = {}) {
    ...
    this.app.get('/api/nearby-networks', async (req, res, next) => {
      try {
        ...
        const nearbyNetworks = await Promise.all(
          panidResult.rows.map(
            (row) => new Promise((resolve, reject) => {
              this.pool.query(
                'SELECT DISTINCT epid FROM wids_networks WHERE panid=$1 '
                  + 'ORDER BY epid',
                [
                  row.panid,
                ],
                (err, epidResult) => {
                  if (err) {
                    ...
                    if (
                      epidResult.rows.length === 1
                      && !epidResult.rows[0].epid
                    ) {
                      networkIdentifiers.push({
                        panid: row.panid,
                        epid: 'Unknown',
                      });
                    } else {
                      for (let i = 0; i < epidResult.rows.length; i += 1) {
                        if (epidResult.rows[i].epid) {
                          networkIdentifiers.push({
                            panid: row.panid,
                            epid: epidResult.rows[i].epid,
                          });
                        }
                      }
                    }
                    resolve(networkIdentifiers);
                  }
                },
              );
            }),
        ...
        res.json(Array.prototype.concat.apply([], nearbyNetworks));
      } catch (err) {
        next(err);
      }
    });
