  ...
  constructor(config = {}) {
    ...
    this.inspectEvents = async () => {
      try {
        const currentEpochTimestamp = Math.floor(Date.now() / 1000.0);
        const selectResult = await this.pool.query(
          'SELECT row_id, wids_sensor_id, description FROM wids_events '
          + 'WHERE inspected=$1 ORDER BY epoch_timestamp',
          [
            false,
          ],
        );
        ...
        for (let i = 0; i < selectResult.rows.length; i += 1) {
          ...
          queryPromises.push(
            new Promise((resolve, reject) => {
              this.pool.query(
                'INSERT INTO nsm_alerts (alert_id, '
                + 'message, archived, notified) '
                + 'VALUES ($1, $2, $3, $4)',
                [
                  alertID,
                  message,
