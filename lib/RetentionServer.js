...
class RetentionServer {
  constructor(config = {}) {
    ...
    this.app.get('/api/archive-folders', async (req, res, next) => {
      try {
        const folderNames = await this.readdir(this.retentionDirectory);
        const folderStats = await Promise.all(folderNames.map(
          (folderName) => this.stat(
            path.join(this.retentionDirectory, folderName),
          ),
        ));
        res.json(folderNames.filter(
          (folderName, i) => folderStats[i].isDirectory(),
        ).sort());
      } catch (err) {
        next(err);
      }
    ...
    this.app.get('/api/archive-folders/:id', async (req, res, next) => {
      ...
      try {
        ...
        const fileNames = await this.readdir(folderPath);
        const fileStats = await Promise.all(fileNames.map(
          (fileName) => this.stat(
            path.join(folderPath, fileName),
          ),
        ));
        res.json(fileNames.filter(
          (fileName, i) => fileStats[i].isFile(),
        ).sort());
      } catch (err) {
        next(err);
      }
    ...
    this.app.get('/api/archive-folders/:id/:file', async (req, res, next) => {
      if (!isValidWIDSSensorID(req.params.id)) {
        res.sendStatus(400);
        return;
      }
    ...
    this.app.get('/api/active-wids-sensors', async (req, res, next) => {
      try {
    ...
    this.app.put('/api/active-wids-sensors', async (req, res, next) => {
      let i;

      try {
        for (i = 0; i < req.body.length; i += 1) {
          if (
    ...
    this.archivePcapFiles = async () => {
      await Promise.all(this.activeWIDSSensors.map(
        (row) => this.fetchPcapFiles(row.wids_sensor_id, row.wids_sensor_api),
      ));
